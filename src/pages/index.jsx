    ...
    <div className='min-h-screen'>
      <div className={`flex items-center justify-between px-4 md:px-12 py-6`}>
        ...
        <div className={`flex items-center space-x-6`}>
          <div onClick={() => history.push('/auth/signin')} className={`border border-brand_blue text-brand_blue px-4 py-2 rounded-lg cursor-pointer hover:bg-brand_blue hover:text-white hover:shadow-lg`}>
            Sign In
          </div>
        </div>
      </div>
      <div className={`h-full flex flex-col justify-center items-center space-y-28 py-24 px-4 md:px-12`}>
        ...
        <div className={`max-w-7xl w-full flex flex-col lg:flex-row justify-between items-center space-y-12 px-6`}>
        ...
        {selected && <Button className={`mx-auto mt-6`} text={`Proceed`} onClick={() => history.push(`/auth/signup/${selected}`)} />}
      </div>
    </div>
  );
};
