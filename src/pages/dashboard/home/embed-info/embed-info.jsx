...
const EmbedInfo = () => {
  ...
  return (
    <div className='w-full h-full py-12 md:py-32 px-4 sm:px-6 md:px-8 flex flex-col items-center justify-center min-h-screen bg-dots'>
      <div className={`flex flex-col justify-center items-center p-4 w-full max-w-3xl space-y-10`}>
        ...
        <div className={`p-10 rounded-xl bg-brand_blue max-w-lg w-full space-y-6 text-gray-100`}>
          <div className={`rounded-xl bg-white bg-opacity-10 h-48`}>
            <textarea
              className={`w-full h-full p-4 bg-transparent focus:outline-none resize-none font-extrabold text-4xl`}
        ...
        <div className={`flex space-x-4 md:space-x-8 justify-center`}>
          <Button className={`w-60`} text={`Back`} type={`secondary`} onClick={() => history.goBack()} />
          <Button className={``} text={`Save And Continue`} onClick={() => history.push('/dashboard/generate-code')} />
        </div>
      </div>
    </div>
  );
