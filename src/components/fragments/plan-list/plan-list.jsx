...
import { ReactComponent as PlanItemDark } from '../../../assets/plan-item-dark.svg';
import { ReactComponent as PlanItemLight } from '../../../assets/plan-item-light.svg';
...
const PlanList = ({ items, onComplete }) => {
  ...
  return (
    <FadeIn className={`flex flex-col lg:flex-row justify-center items-center w-full`}>
      {items.map((item, idx) => {
  ...
  return (
    <div
      ...
      className={`${selected ? `card` : ``} flex flex-col space-y-8 rounded-3xl max-w-lg cursor-pointer w-full divide-y divide-gray-300 transform ${
        selected ? `border-4 border-white bg-brand_blue p-12 shadow-2xl text-white z-10` : `bg-white shadow-sm p-6 text-gray-600 scale-95`
      }`}
    >
      <div className={`flex space-x-4 items-center`}>
        {selected ? (
          <div className={`w-20`}>
            <PlanItemDark className={`w-full`} />
          </div>
        ) : (
          <div className={`w-20`}>
            <PlanItemLight className={`w-full`} />
          </div>
        )}
      ...
      <div className={`pt-8 space-y-8`}>
        ...
        {selected ? (
          <Button className={`w-full`} light={true} text={`Choose Plan`} onClick={() => handleButtonClick(selected)} />
        ) : (
